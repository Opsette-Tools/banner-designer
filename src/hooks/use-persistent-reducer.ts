...
export function usePersistentReducer<S extends object, A>(
  ...
  reducer: Reducer<S, A>,
  initial: S,
  // Optional custom merge of the stored blob onto `initial`. Needed when state
  // has NESTED objects (a shallow `{...base, ...saved}` would drop default keys
  // inside them). Defaults to the shallow merge for the common flat case.
  hydrate?: (saved: Partial<S>) => S,
): [S, React.Dispatch<A>] {
  const [state, dispatch] = useReducer(reducer, initial, (base) => {
    ...
    try {
      ...
      return hydrate ? hydrate(saved) : { ...base, ...saved };
    } catch {
      return base;
    }
  });
